 ...
 * Copyright (c) Andy Goryachev <andy@goryachev.com>
 *
...
public class JEditTextArea
    extends JComponent
{
	public final CAction copyAction = new CAction() { public void action() { copy(); } };
	public final CAction cutAction = new CAction() { public void action() { cut(); } };
	public final CAction pasteAction = new CAction() { public void action() { paste(); } };
	// TODO undo/redo
	...
	protected static Timer caretTimer = initTimer();
	/**
	...
	public JEditTextArea(TextAreaDefaults defaults)
	{
		...
		add(CENTER, painter);
		add(RIGHT, vertical = createScrollBar(JScrollBar.VERTICAL));
		add(BOTTOM, horizontal = createScrollBar(JScrollBar.HORIZONTAL));
	...
	{
		Timer t = new Timer(500, new CaretBlinker());
	 ...
	 */
	public void setDocument(SyntaxDocument d)
	{
		if(this.document == d)
		{
			return;
		...
		d.addDocumentListener(documentHandler);
	...
	protected void updateBracketHighlight(int newCaretPosition)
	{
		...
		try
		{
			int offset = JEditUtilities.findMatchingBracket(document, newCaretPosition - 1);
			if(offset != -1)
			{
	...
	public static class CaretBlinker
	...
	public class MutableCaretEvent
	    extends CaretEvent
	{
	    ...
	    implements MouseMotionListener
	{
		public void mouseDragged(MouseEvent ev)
		{
			...
			setSelectionRectangular((ev.getModifiers() & InputEvent.CTRL_MASK) != 0);
			select(getMarkPosition(), xyToOffset(ev.getX(), ev.getY()));
		}
	    ...
	    extends MouseAdapter
	{
		public void mousePressed(MouseEvent ev)
		{
			...
			if((ev.getModifiers() & InputEvent.BUTTON3_MASK) != 0 && popup != null)
			{
				popup.show(painter, ev.getX(), ev.getY());
				return;
			}

			int line = yToLine(ev.getY());
			int offset = xToOffset(line, ev.getX());
			int dot = getLineStartOffset(line) + offset;

			switch(ev.getClickCount())
			{
			case 1:
				doSingleClick(ev, line, offset, dot);
				break;
			case 2:
				...
				try
				{
					doDoubleClick(ev, line, offset, dot);
				}
				catch(Exception e)
				{
					Log.err(e);
				...
				break;
			case 3:
				doTripleClick(ev, line, offset, dot);
				break;
			}
		}


		protected void doSingleClick(MouseEvent ev, int line, int offset, int dot)
		{
			if((ev.getModifiers() & InputEvent.SHIFT_MASK) != 0)
			{
				rectSelect = (ev.getModifiers() & InputEvent.CTRL_MASK) != 0;
		...
		protected void doDoubleClick(MouseEvent ev, int line, int offset, int dot) throws BadLocationException
		{
			// Ignore empty lines
			...
			try
			{
				int bracket = JEditUtilities.findMatchingBracket(document, Math.max(0, dot - 1));
				if(bracket != -1)
				{
		...
		protected void doTripleClick(MouseEvent ev, int line, int offset, int dot)
		{
	    ...
	    extends AbstractUndoableEdit
	{
		...
		public CaretUndo(int start, int end)
		{
			this.start = start;
			this.end = end;
		}
